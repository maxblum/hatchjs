...
PermissionsAPI.prototype.getParent = function(name) {
    if(name.indexOf('.') == -1) return this.root;
...
PermissionsAPI.prototype.match = function(permissionList, name) {
    ...
    _.forEach(permissionList, function(permissionName) {
        if(permissionName == name || name.indexOf(permissionName + '.') > -1) found = true;

        //check for wildcard permissions
        if(name.indexOf('.*') > -1 && new RegExp(name).exec(permissionName)) found = true;
    });
