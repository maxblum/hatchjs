...
HooksAPI.prototype.hook = function(c, name, params, next, after) {
    ...
    // filter the subscribers by whether their calling module is enabled for
    // the group in the request
    if (subscribers) {
        subscribers = _.filter(subscribers, function(subscriber) {
            return !subscriber.module || group.getModule(subscriber.module);
        });
    }

    $.puts($('Hook.fire:').green, $(name).blue, '\n');
...
HooksAPI.prototype.wrap = function(name, originalFn, after) {
    ...
    return function() {
        ...
        // always assume context is the first argument - it usually is when it
        // is needed
        var c = args[0];

        if (typeof c !== 'object' || !c.req) {
            return originalFn.apply(obj, args);
        }

        hooks.hook(c, name, arguments, function() {
