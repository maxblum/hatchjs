...
SpecialPage.prototype.path = function path(group, params) {
    ...
    var homepage = group ? group.homepage.url : '';

    var url = found || homepage.replace(/\/$/, '') + '/' + this.defaultPath;
...
SpecialPage.prototype.matchRoute = function matchRoute(group, path) {
    ...
    var params = query ? $url.parse('?' + query, true).query : {};
    var m;
    ...
    if (m = path.match(re) || path.match(re2)) {
        m.slice(1).forEach(function (token, i) {
            ...
            params[paramNames[i]] = params[paramNames[i]] || token;
        });
        return params;
    } else {
        return false;
