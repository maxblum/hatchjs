...
var WidgetAPI = require('./api/widget.js');
var ImportStreamAPI = require('./api/importStream.js');
var HooksAPI = require('./api/hooks.js');
var NotificationAPI = require('./api/notification.js');
var PageAPI = require('./api/page.js');
var ErrorsAPI = require('./api/errors.js');
var ModelContextAPI = require('./api/modelContext.js');
var UploadAPI = require('./api/upload.js');
var ThemesAPI = require('./api/themes.js');
var CryptoAPI = require('./api/crypto.js');
var ContentTypeAPI = require('./api/contentType.js');
var PermissionsAPI = require('./api/permissions.js');
...
function HatchPlatform(app) {
    ...
    this.contentType = new ContentTypeAPI(this);
    this.permissions = new PermissionsAPI(this);
    this.search = new SearchAPI(this, 'reds');
    this.exportData = new ExportAPI(this);
    this.mediator = express();
    this._i18n = {};

    var pauseLocalesBundle;
    this.i18nUpdate = function(compound) {

        cloneTo(hatch._i18n, compound.__localeData);
        if (pauseLocalesBundle) clearTimeout(pauseLocalesBundle);
        pauseLocalesBundle = setTimeout(function() {
            var t = fs.readFileSync(__dirname + '/i18n-template.js').toString();
            // console.log('writing /javascripts/i18n.js');
            fs.writeFileSync(
                __dirname + '/../public/javascripts/i18n.js',
                t.replace('{{ i18n }}', JSON.stringify(app.compound.hatch._i18n)));
        }, 3000);
    };
    var self = this;
    app.compound.on('ready', function () { self.i18nUpdate(app.compound) });
};
...
HatchPlatform.prototype.loadModuleFromPath = function(m, modPath) {
    ...
    mod.compound.on('routes', function () {
        if (!mod.compound.injectMiddlewareBefore(mod.router, exports.middleware(compound))) {
            mod.use(mod.router);
        }
    });


    mod.compound.on('ready', hatch.i18nUpdate);
...
exports.modules = function (compound) {
    ...
    compound.controllerExtensions.moduleConfigured = function (moduleName) {
        var module = this.moduleEnabled(moduleName);
        var info = compound.hatch.modules[moduleName].info;
...
function loadCommonStuff(structure, compound) {
    ...
    structure.helpers.application_helper.moduleConfigured = exts.moduleConfigured;
...
exports.middleware = function (compound) {
    ...
    function loadEnvForAPI(req, res, next) {
        ...
        req.user = req.user || data && new User(data.user);
        req.data = data;

        if (req.locals.group || !data) {
            return next();
        }
