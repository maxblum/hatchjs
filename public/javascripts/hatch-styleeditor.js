...
function StyleEditorController() {
    ...
    this.els = {
        ...
        selectedLabel: $("#style-selected-element"),
    ...
    function selectElement(el, selector) {
        ...
        if(allSelectors.length > 1) {
            ...
            var $menu = this.els.similarElements.find('ul.dropdown-menu');
            $menu.empty();

            allSelectors.forEach(function(sel) {
                if(sel != selector) {
                    var $li = $('<li><a href="#">' + sel + '</a></li>');
                    var $a = $li.find('a');
    ...
    function init() {
        ...
        for (var x = 0; x < document.styleSheets.length; x++) {
            ...
            for (var i = 0; i < rules.length; i++) {
                ...
                var ignore = false;
                for(j in disallowedSelectors) if(rule == null || rule.indexOf(disallowedSelectors[j]) > -1) ignore = true;
                //if(ignore) continue;
        ...
        c.els.themeSelect.bind("change", function() {
            var theme = c.els.themeSelect.val();
            c.els.themePreview.attr("src", $(c.els.themeSelect[0].options[c.els.themeSelect[0].selectedIndex]).attr('data-thumbnail'));
    ...
    function addAdvancedProperty(property, value) {
        ...
        var width = 230 - 11 - (6 * property.length) - 16;
        ...
        input.bind("keydown", function(e) {
            ...
            if(delta != 0) {
                ...
                var cursor = originalCursor = input[0].selectionStart;
    ...
    function setCssRule(selector, property, value) {
        ...
        if(selector != "@") {
            ...
            if($(selector).css(property) == value) return;
